...
var createVirtualMachine = async (options) => {
  const {
    name,
    iso = "koompi",
    ram = "1024",
  ...
  try {
    ...
    console.log(`Virtual machine "${name}" not exists`);
  }
  try {
    if (!ram.match(/^\d+$/)) {
    ...
    const diskFile = `images/${name}.qcow2`;
...
// vm/removeVirtualMachine.ts
var removeVirtualMachine = async (options) => {
...
// vm/startVirtualMachine.ts
var startVirtualMachine = async (options) => {
  const { name = "koompi" } = options;
  try {
    const vmExistsOutput = await executeCommand(`virsh dominfo ${name}`);
    if (!vmExistsOutput.includes("Name:")) {
...
// vm/stopVirtualMachine.ts
var stopVirtualMachine = async (options) => {
  const { name = "koompi" } = options;
  try {
    const vmExistsOutput = await executeCommand(`virsh dominfo ${name}`);
    if (!vmExistsOutput.includes("Name:")) {
    ...
    if (vmStatusOutput.includes("running")) {
      ...
      await executeCommand(`virsh destroy ${name} --graceful`);
...
// shells/getStatusFromOutput.ts
function getStatusFromOutput(output) {
  const lines = output.split("\n");
...
var removeVM = async (req, res) => {
  try {
    const vmOptions = {
      name: req.body.name || "default_name"
    };
    const removeVM2 = await removeVirtualMachine(vmOptions);
    res.status(201).json({ message: "VM successfully removed", vm: removeVM2 });
  } catch (error) {
    console.error("Virtual machine not found:", error);
    res.status(500).json({ message: "Virtual machine not found", error });
  }
};
var startVM = async (req, res) => {
  try {
    const vmOptions = {
      name: req.body.name || "default_name"
    };
    const startVM2 = await startVirtualMachine(vmOptions);
    res.status(201).json({ message: "Virtual Machine is starting", vm: startVM2 });
  } catch (error) {
...
var stopVM = async (req, res) => {
  try {
    const vmOptions = {
      name: req.body.name || "default_name"
    };
    const stopVM2 = await stopVirtualMachine(vmOptions);
    res.status(201).json({ message: "Virtual Machine is stopping", vm: stopVM2 });
  } catch (error) {
    console.error("Virtual machine not found:", error);
    res.status(500).json({ message: "Virtual machine not found", error });
  }
};
var checkInfoVM = async (req, res) => {
  try {
    const vmOptions = {
      name: req.body.name || "default_name"
    };
